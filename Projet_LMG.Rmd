...
p2=ggplot(data=d.train, mapping=aes(x = as.factor(Month), y = Precip.tot)) +
  geom_boxplot(fill="deepskyblue3", width = 1) + 
...
ggplot(data=d.train, mapping=aes(x = as.factor(Month), fill=pluie.auj)) +
  ...
  scale_x_discrete(labels=c("Jan","Fev","Mar","Avr","Mai","Juin","Juil","Aou", "Sep", "Oct","Nov","Dec")) + 
 scale_fill_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse"))
```
...
D'après ces observations graphiques, on a l'impression que la pression athmosphérique, la nébulosité, l'ensoleillement, les caractéristiques du vent d'une journée ont un impact sur la prévision météo pour le lendemain.
...
p4=ggplot(data=d.train, mapping=aes(x = Precip.tot,y=Temp.mean,col=pluie.demain)) +
  ...
  geom_point() +
scale_color_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse")) 
...
p16=ggplot(data=d.train, mapping=aes(x = QuartWDir10m, y =Precip.tot )) +
  labs(subtitle="graphe 16 ", y="Précipitation du jour (mm)", x="Dir.à 10m altitude") + geom_boxplot(fill="deepskyblue3") 

p17=ggplot(data=d.train, mapping=aes(x = QuartWDir80m, y =Precip.tot )) +
  labs(subtitle="graphe 17", y="", x="Dir.à 80m altitude") + geom_boxplot(fill="deepskyblue3")

p18=ggplot(data=d.train, mapping=aes(x = QuartWDir900mb, y =Precip.tot )) +
  labs(subtitle="graphe 18", y="", x="Dir.à pression de 900hPA") + geom_boxplot(fill="deepskyblue3")
...
Il s'agit maintenant de trouver un modèle qui explique de façon satisfaisante la prévision "pluie.demain". Nous avons à disposition 46 variables explicatives. Il est donc fastidieux de tester tous les modèles possibles.
Essayons de vérifier quelques corrélations intuitives permettant de ne pas sélectionner des variables redondantes.
...
Des corrélations fortes (coef de corrélation proche de 1 ) existent entre : 
...
## Méthode "exhaustive" 
Dans un premier temps, utilisons une méthode de choix de modèle "manuelle" en sélectionnant les variables explicatives semblant pertinentes sur la base des influences et correlations observées. Les critères CP Mallows, BIC permettent d'identifier ds variables explicatives pertinentes (variables avec CP Mallows minimal / BIC minimal).
...
**Modèle 1** : 10 variables explicatives (choisies à partir des corrélations et influences observées précédemment).  
  ...
  + ensoleillement  
  + rafales de vent minimum et maximum
...
plot(choixman_mod1,scale = "Cp",col="chartreuse", main="Modèle 1 - Cp Mallows")
plot(choixman_mod1,scale = "bic", col="chartreuse", main="Modèle 1 - BIC")
...
plot(choixman_mod2,scale = "Cp",col="chartreuse", main="Modèle 2 - Cp Mallows")
plot(choixman_mod2,scale = "bic", col="chartreuse", main="Modèle 2 - BIC")

```


**Résultat Modèle 2** :  
Après observation des résultats sur les modèles 1 et 2, on constate que les 4 variables les plus pertinentes sont la pression atmosphérique, les nébulosités totale et forte, la direction du vent sous 900HPa.   
...
plot(choixman_mod3,scale = "Cp",col="chartreuse", main="Modèle 3 - Cp Mallows")
plot(choixman_mod3,scale = "bic", col="chartreuse", main="Modèle 3 - BIC")
...
plot(choixman_mod4,scale = "Cp",col="chartreuse", main="Modèle 4 - Cp Mallows")
plot(choixman_mod4,scale = "bic", col="chartreuse", main="Modèle 4 - BIC")
```


Testons un modèle plus complet en incluant le niveau de chite de neige et l'humidité moyenne.
...
-> choix de la variable :  **direction du vent sous 900HPa**
    
*Etape 3* :   
-> choix de la variable : **nébulosité forte**
    
*Etape 4* : 
-> choix de la variable : **nébulosité totale**
  
*Etape 5* : 
-> choix de la variable **rafale maximum**
  
*Etape 6* : 
-> choix de la variable **vitesse du vent à 10m **
...
**Résultat Modèle 1** : 6 variables sont sélectionnées = *Modèle 1 réduit*
...
mod1.cpl=glm(pluie.demain.bin~Pressure.mean+Precip.tot+TotalCloud.mean+HighCloud.mean+WindSpeed10m.mean+WindDir10m.mean+WindDir900mb.mean+Sunshine.tot+WindGust.max+WindGust.min,data=d.train,family=binomial)

mod1=glm(pluie.demain.bin~Pressure.mean+TotalCloud.mean+HighCloud.mean+WindSpeed10m.mean+WindDir900mb.mean+WindGust.max,data=d.train,family=binomial)
...
**Résultat Modèle 2** : 5 variables sont selectionnées = *Modèle 2 réduit*
...
```{r choixprog_mod2, include=FALSE}
# modèle 2
...
mod2.cpl=glm(pluie.demain.bin~Pressure.mean+TotalCloud.mean+HighCloud.mean+WindSpeed900mb.max+WindDir10m.mean+WindDir900mb.mean+WindGust.max+WindGust.min,data=d.train,family=binomial)

mod2=glm(pluie.demain.bin~Pressure.mean+TotalCloud.mean+HighCloud.mean+WindSpeed900mb.max+WindDir900mb.mean,data=d.train,family=binomial)
...
```{r choixprog_mod3,include=FALSE}
...
**Résultat Modèle 4** : 7 variables sont selectionnées = *Modèle 4 réduit*
...
```{r choixprog_mod4, include=FALSE}
...
mod4.cpl=glm(pluie.demain.bin~Pressure.mean+Shortwave.tot+TotalCloud.mean+HighCloud.mean+WindSpeed900mb.max+WindDir10m.mean+WindDir900mb.mean+WindGust.max+WindGust.min,data=d.train,family=binomial)

mod4=glm(pluie.demain.bin~Pressure.mean+Shortwave.tot+TotalCloud.mean+HighCloud.mean+WindSpeed900mb.max+WindDir10m.mean+WindDir900mb.mean,data=d.train,family=binomial)
...
**Résultat Modèle 5** : 8 variables sont selectionnées = *Modèle 5 réduit* 
...
```{r choixprog_mod5,include=FALSE}
...
mod5.cpl=glm(pluie.demain.bin~Pressure.mean+Shortwave.tot+TotalCloud.mean+HighCloud.mean+WindDir10m.mean+WindDir900mb.mean+WindGust.max+Snow.tot,data=d.train,family=binomial)

mod5=glm(pluie.demain.bin~Pressure.mean+Shortwave.tot+TotalCloud.mean+HighCloud.mean+WindSpeed900mb.max+WindDir10m.mean+WindDir900mb.mean+Snow.tot,data=d.train,family=binomial)
...
**Les 5 modèles sont réduits aux variables selectionnées dans la méthode progressive.**  

**AIC et déviance des modèles réduits**

```{r crit}

table_res2=data.frame(Modele=c("Modèle nul","Modèle 1 réduit","Modèle 2 réduit","Modèle 3","Modèle 4 réduit","Modèle 5 réduit"),AIC=c(aic_mod0,aic_mod1,aic_mod2,aic_mod3,aic_mod4,aic_mod5),Deviance=c(deviance_mod0,deviance_mod1,deviance_mod2,deviance_mod3,deviance_mod4,deviance_mod5),Df=c(dfdeviance_mod0,dfdeviance_mod1,dfdeviance_mod2,dfdeviance_mod3,dfdeviance_mod4,dfdeviance_mod5))
                     
kable(head(table_res2), format="markdown", digits=0)

```

**Résultat de la méthode progressive** :   
Le meilleur modèle est le 5 (le critère AIC le plus bas). Les modèles 1 et 4 sont également de bons modèles : ils affichent un critère AIC légèrement supérieur. 
...
table_res3=data.frame(Modele=c("Modèle 1 réduit","Modèle 2 réduit","Modèle 3","Modèle 4 réduit","Modèle 5 réduit"),pvalue.Deviance.nulle=c(pchisq0_mod1,pchisq0_mod2,pchisq0_mod3,pchisq0_mod4,pchisq0_mod5),pvalue.Deviance.saturee=c(pchisqsat_mod1,pchisqsat_mod2,pchisqsat_mod3,pchisqsat_mod4,pchisqsat_mod5))
                     
kable(head(table_res3), format="markdown", digits=4)

```

**Résultat du test de déviance** :   
Les modèles testés sont meilleurs que le modèle nul (sans covariable) mais moins bons que le modèle saturé (46 variables).
...
Il s'agit de comparer 2 modèles imbriqués et de déterminer le "meilleur" des 2 modèles à partir de la p_value du test. si la p-value est < au seuil (ex: 5%) alors le modèle avec le plus de covariables est le meilleur.
 ...
 - Modèle 1 vs Modèle 3 :   
 p-valeur < 5%, le meilleur modèle est celui avec le plus de covariables : le modèle 1
...
``` 
**Résultat des tests anova** : les modèles avec le plus de covariables (1, 4, 5) sont toujours préférés au modèle minimal (modèle 3). Le modèle 5 affiche la plus petite p-valeur du test anova vs modèle 3. 

##Validation croisée  

Il s'agit de comparer 2 modèles à partir de leur précision prédictive ; plus la prédiction sera proche de la vraie valeur , meilleure sera la capacité prédictive du modèle.  La mesure de l'erreur de prédiction est :  
**erreur prédiction = moyenne (|prédiction - vraie valeur|)**

Les prédictions sont calculées pour chaque modèle réduit sur un echantillon de 80% de la base d'entrainement. Elles sont comparées aux vraies valeurs de 20% de l'échantillon de la base d'entrainement.
...
# modèle 1 réduit sur 80% de base entrainement
mod1.80=glm(
  pluie.demain.bin~Pressure.mean+TotalCloud.mean+HighCloud.mean+WindSpeed10m.mean+WindDir900mb.mean+WindGust.max,data=d.train[train_80,],
  family=binomial
  )
# prédiction sur 20% base test
pred_mod1=predict(mod1.80,d.train[!train_80,],type="response")## erreur de prédiction
# erreur de prédiction
mean_mod1=mean(abs(pred_mod1-d.train[!train_80,"pluie.demain.bin"]),na.rm=T)

#modèle 2 réduit sur 80% de base entrainement 
mod2.80=glm(pluie.demain.bin~Pressure.mean+TotalCloud.mean+HighCloud.mean+WindSpeed900mb.max+WindDir900mb.mean,data=d.train[train_80,],family=binomial)
# prédiction sur 20% base test
...
#modèle 3 sur 80% de base entrainement
...
#modèle 4 réduit sur 80% de base entrainement
mod4.80=glm(pluie.demain.bin~Pressure.mean+Shortwave.tot+TotalCloud.mean+HighCloud.mean+WindSpeed900mb.max+WindDir10m.mean+WindDir900mb.mean,data=d.train[train_80,],family=binomial)
# prédiction sur 20% base test
...
#modèle 5 réduit sur 80% de base entrainement
mod5.80=glm(pluie.demain.bin~Pressure.mean+Shortwave.tot+TotalCloud.mean+HighCloud.mean+WindSpeed900mb.max+WindDir10m.mean+WindDir900mb.mean+Snow.tot,data=d.train[train_80,],family=binomial)
# prédiction sur 20% base test
...
table_res4=data.frame(Modele=c("Modèle 1 réduit" ,"Modèle 3","Modèle 4 réduit","Modèle 5 réduit"),erreur=c(mean_mod1,mean_mod3,mean_mod4,mean_mod5))
                     
kable(head(table_res4), format="markdown", digits=4)

```

**Résultat de la validation croisée** : les erreurs prédictives des 4 modèles sont proches quelque soit l'échantillon tiré. Mais la plus basse n'est pas obtenue par le meme modèle à chaque tirage. 

##Caractéristiques du modèle choisi  

Le modèle 5 réduit apparait comme le meilleur modèle : critère AIC le plus faible, p-valeur des tests anova vs modèle 3 la plus faible.   
Il a les caratéristiques suivantes :

```{r affichecoef}
#kableExtra::kable(summary(mod5)$coefficients, format="markdown", digits=6) %>%
#kable_styling(full_width = F,)
kable(summary(mod5)$coefficients, format="markdown", digits=6)

```

Les coefficients sont significatifs (p-value < 5%) concernant toutes les covariables sauf le niveau de chute de neige et la direction du vent à 10m. Leur influence est positive sur la probabilité d'anticiper de la pluie pour le lendemain, sauf pour la pression atmosphérique : l'augmentation de la pression atmosphérique du jour diminue la probabilité de prévoir de la pluie pour le jour d'après. 
   
# Prévision de la météo du lendemain 

Les prévisions de pluie sont calculées à partir du modèle 5 réduit sur les données du fichier test *meteo.test.csv*    
(fichier en sortie : *Meteo pred.csv*)


```{r predic}

prev=predict(mod5,newdata=d.test,type = "response")
d.test$pluie.demain[(prev>=0.5)]=TRUE
d.test$pluie.demain[(prev<0.5)]=FALSE
write.csv2(d.test, "Meteo pred.csv")
```

**Caractéristiques des prévisions calculées**

Sur l'ensemble du fichier test (277 journées entre janvier 2011 et juin 2018), la prévision  de jours avec pluie est de 50.2%. Les prévisions de pluie importantes sont sur les mois de mars, mai, juin et juillet ; les moins importantes sont celles pour les mois de septembre, octobre et décembre. (résultats proches des observations cf. 2.1, sauf sur mars )

Les graphiques présentant l'influence des variables explicatives sur la prévision météo du lendemain réalisés cette fois sur les données de la base de test montrent des résultats proches des graphiques précédents (graphes 4 à 15 réalisés sur la base d'entrainement, cf. 2.2).
...
# influence de la température
p4b=ggplot(data=d.test, mapping=aes(x = Precip.tot,y=Temp.mean,col=pluie.demain)) +
  labs(subtitle="graphe 4b", x="Précipitation du jour (mm)", y="Température (°C)",col="Prévision lendemain") +
  geom_point() +
scale_color_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse")) 

p5b=ggplot(data=d.test, mapping=aes(x = pluie.demain, y = Temp.mean, fill = pluie.demain)) +
  geom_boxplot() + 
  labs(subtitle="graphe 5b",x="",y="Température (°C)",fill="Prévision lendemain" ) + 
  scale_x_discrete(labels=c("","")) +  
  scale_fill_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse"))

grid.arrange(p4b,p5b,ncol=2,top="  Influence de la température du jour sur la prévision météo du lendemain")
  
# influence Précipitation et humidité
p6b=ggplot(data=d.test, mapping=aes(x = Precip.tot,y=Humid.mean,col=pluie.demain)) +
  labs(subtitle="graphe 6b", x="Précipitation du jour (mm)", y="Humidité (%)",col="Prévision du lendemain") +
  geom_point() +
scale_color_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse")) 

p7b=ggplot(data=d.test, mapping=aes(x = pluie.demain, y = Humid.mean, fill = pluie.demain)) +
  geom_boxplot() + 
  labs(subtitle="graphe 7b",x="",y="Humidité (%)",fill="Prévision lendemain" ) + 
  scale_x_discrete(labels=c("","")) +  
  scale_fill_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse"))

grid.arrange(p6b,p7b,ncol=2,top="Influence de l'humidité du jour sur la prévision météo du lendemain")

# influence Précipitation et pression atmosphérique
p8b=ggplot(data=d.test, mapping=aes(x = Precip.tot,y=Pressure.mean,col=pluie.demain)) +
  labs(subtitle="graphe 8b", x="Précipitation du jour (mm)", y="Pression (hPA)",col="Prévision du lendemain") +
  geom_point() +
scale_color_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse")) 

p9b=ggplot(data=d.test, mapping=aes(x = pluie.demain, y = Pressure.mean, fill = pluie.demain)) +
  geom_boxplot() + 
  labs(subtitle="graphe 9b",x="",y="Pression (hPA)",fill="Prévision lendemain" ) + 
  scale_x_discrete(labels=c("","")) +  
  scale_fill_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse"))

grid.arrange(p8b,p9b,ncol=2,top="Influence de la pression atmosphérique du jour sur la prévision météo du lendemain")

# influence Précipitation et nébulosité
p10b=ggplot(data=d.test, mapping=aes(x = Precip.tot,y=TotalCloud.mean,col=pluie.demain)) +
  labs(subtitle="graphe 10b", x="Précipitation du jour (mm)", y="Nébulosité (%)",col="Prévision du lendemain") +
  geom_point() +
scale_color_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse")) 

p11b=ggplot(data=d.test, mapping=aes(x = pluie.demain, y = TotalCloud.mean, fill = pluie.demain)) +
  geom_boxplot() + 
  labs(subtitle="graphe 11b",x="",y="Nébulosité (%)",fill="Prévision lendemain" ) + 
  scale_x_discrete(labels=c("","")) +  
  scale_fill_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse"))

grid.arrange(p10b,p11b,ncol=2,top="Influence de la nébulosité du jour sur la prévision météo du lendemain")

# influence Précipitation et ensoleillement

p12b=ggplot(data=d.test, mapping=aes(x = Precip.tot,y=Sunshine.tot,col=pluie.demain)) +
  labs(subtitle="graphe 12b", x="Précipitation du jour (mm)", y="Ensoleillement (minute)",col="Prévision du lendemain") +
  geom_point() +
scale_color_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse")) 

p13b=ggplot(data=d.test, mapping=aes(x = pluie.demain, y = Sunshine.tot, fill = pluie.demain)) +
  geom_boxplot() + 
  labs(subtitle="graphe 13b",x="",y="Ensoleillement (minute)",fill="Prévision lendemain" ) + 
  scale_x_discrete(labels=c("","")) +  
  scale_fill_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse"))

grid.arrange(p12b,p13b,ncol=2,top="Influence de l'ensoleillement du jour sur la prévision météo du lendemain")

# influence Précipitation et rafale de vent
p14b=ggplot(data=d.test, mapping=aes(x = Precip.tot,y=WindGust.max,col=pluie.demain)) +
  labs(subtitle="graphe 14b", x="Précipitation du jour (mm)", y="Rafale de vent max (km/h)",col="Prévision du lendemain") +
  geom_point() +
scale_color_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse")) 

p15b=ggplot(data=d.test, mapping=aes(x = pluie.demain, y = WindGust.max, fill = pluie.demain)) +
  geom_boxplot() + 
  labs(subtitle="graphe 15b",x="",y="Rafale de vent max(km/h)",fill="Prévision lendemain" ) +
  scale_x_discrete(labels=c("","")) +  
  scale_fill_manual(values=c("yellow","deepskyblue3"),label=c("journée sans pluie","journée pluvieuse"))

grid.arrange(p14b,p15b,ncol=2,top="Influence des rafales de vent du jour sur la prévision météo du lendemain")

```

# Les bords du Rhin à Bâle au soleil et sous la pluie ...

<center>                         
![](C:\Users\Marlène\Desktop\Projet R - MLG\Bale5-2.jpg)

       
                  
![](C:\Users\Marlène\Desktop\Projet R - MLG\Bale4-pluie-2.jpg)
</center>
