...
**Déroulement de la sélection de variables explicatives** :  
   ...
   **=> choix de la variable direction du vent sous 900HPa**
   ...
   - Etape 3 : **=> choix de la variable nébulosité forte**
    
   - Etape 4 : **=> choix de la variable nébulosité totale**
  
   - Etape 5 : **=> choix de la variable rafale maximum**
  
   - Etape 6 : **=> choix de la variable vitesse du vent à 10m **
...
``` 
**Résultat test anova** : les modèles avec le plus de covariables (1, 4, 5) sont jours préférés au modèle minimal (modèle 3).Le modèle 5 affiche la plus petite p-valeur du test anova vs modèle 3. 
...
Avec le modèle 5, les erreurs de prédiction sont les plus basses.

##Caractéristiques du modèle choisi  

Le modèle 5 (réduit) apparait comme le meilleur modèle . Il a les caratéristiques suivantes :
...
Les coefficients sont significatifs (p-value < 5%) concernant toutes les covariables sauf le niveau de chute de neige et la direction du vent à 10m. Leur influence est positive sur la probabilité d'anticiper de la pluie lelendemain, sauf pour la pression atmosphérique : l'augmentation de la pression atmosphérique du jour diminue la probabilité de prévoir de la pluie pour le jour d'après. 
...
Sur l'ensemble du fichier test (277 journées entre janvier 2011 et juin 2018), la prévision  de jours avec pluie est de 50.2%. Les prévisions de pluie importantes sont sur les mois de mars, mai, juin et juillet ; les moins importantes sont celles pour les mois de septembre, octobre et décembre. (résultats proches des observations, sauf sur mars )

Les graphiques présentant l'influence des variables explicatives sur la prévision météo du lendemain réalisés cette fois sur les données de la base de test montrent des résultats proches des graphiques précédents (graphes 4 à 15 réalisés sur la base d'entrainement).
