...
```
Des corrrélations fortes existent entre :  
   - les directions du vent à 10m et à 80m    
   - les vitesses de vent à 10m, à 80m et ous 900HPa  
...
Illustration de la démarche  :
  ...
  - **Modèle 1** : 7 variables explicatives   
       ...
       + humidité moyenne, 
       + nébulosité globale,
       + direction de vent à 10m altitude,
       + vitesse de vent à 10m altitude, 
       + ensoleillement
...
```
  ...
  - **Modèle 4** : 5 variables explicatives (= modèle 3 + humidité moyenne)
    + presssion moyenne,   
    + nébulosité totale,   
    + vitesse maximum du vent sous 900hPA,  
    + direction du vent sous 900hPA,  
...
```
  ...
  - **Modèle 5** : 5 variables explicatives (= modèle 3 + température moyenne)
    + presssion moyenne,   
    + nébulosité totale,   
    + vitesse maximum du vent sous 900hPA,  
    + direction du vent sous 900hPA,  
...
```
   ...
   - **Modèle 6 ** : 3 variables explicatives (modèle 3 - vitesse du vent)
     + presssion moyenne,   
     + nébulosité totale,   
     + direction du vent sous 900hPA  
...
Quelque soit l'echantillon aléatoire (=80% des individus du fichier d'entrainement), les modèles 3, 4,5,6 sont meilleurs (AIC sont proches et sont plus bas que le modele sans variable explicative et que le modèle 1)
...
```
  ...
  - Modèle 1  
   Deviance nulle :`r pchisq0_mod1`
   Deviance saturée :`r pchisqsat_mod1`
   
  - Modèle 3  
   Deviance nulle :`r pchisq0_mod3`
   Deviance saturée :`r pchisqsat_mod3`
   
   - Modèle 4  
   Deviance nulle :`r pchisq0_mod4`
   Deviance saturée :`r pchisqsat_mod4`
   
   - Modèle 5  
   Deviance nulle :`r pchisq0_mod5`
   Deviance saturée :`r pchisqsat_mod5`
   
   - Modèle 6  
   Deviance nulle :`r pchisq0_mod6`
   Deviance saturée :`r pchisqsat_mod6`
...
pred_mod3=predict(mod3,d.train[!train_80,],type="response")
mean_mod3=mean(abs(pred_mod3-d.train[!train_80,"pluie.demain.bin"]),na.rm=T)
mean_mod3
table(pred_mod3,d.train[!train_80,"pluie.demain.bin"])
#prédiction sur le modèle 6
pred_mod6=predict(mod6,d.train[!train_80,],type="response")
mean_mod6=mean(abs(pred_mod6-d.train[!train_80,"pluie.demain.bin"]),na.rm=T) 
mean_mod6
