...
Examinons maintenant le comportement des variables météorologiques au regard de la prédiction qui a été faite pour le lendemain.
  ...
  - **La durée d'enseillement du jour** a également une influence sur les prévisions du lendemain : moins l'ensoleillement est important, plus on aura tendance à prévoir de la pluie pour le lendemain (*cf. graphe 13* : le niveau médian du temps d'ensoleillement ).
   ...
   - **Le sens et la direction du vent** de la journée semblent impacter significativement les prévisions pour le lendemain (*cf. graphes 16 à 24*). Les vents du nord et d'ouest amènent des pluie plus abondantes que les vents du sud et d'est (*cf. graphes 16 à 18*).
Le vent d'ouest quelque soit leur vitesse sont souvent signe de pluie en journée et conduit à prévoir une journée du lendemain pluvieuse (*cf. graphes 19 à 24*).
...
p18=ggplot(data=d.train, mapping=aes(x = QuartWDir900mb, y =Precip.tot )) +
  labs(subtitle="graphe 18", y="", x="Dir.à pression de 900hPA") + geom_boxplot(fill="lightblue")
...
Testons les corrélations entre les variables de nébulosités, de Vitesse de vent, de directions du vents.
Il apparait qu'il y a des corrélations statistiquement significatives (p-value de cor.test<5%) entre :
    - les variables de direction du vent (entre 10m, 80m d'altitude et sous 900HPA)
    - les variables de vitesse du vent (entre 10m, 80m d'altitude, sous 900HPA, rafale de vent)
    - les variables de nébulosité (totale/forte/medium/faible)
    
## Méthode "exhaustive" 
Dans un premier temps, utilisons une méthode de choix de modèle "manuelle" en sélectionnant les variables explicatives semblant pertinentes sur la base des influences et correlations observées. Les critères R2, R2 ajusté, CP Mallows, BIC permettront d'identifier les variables à choisir. (min CP Mallows et min BIC)

Illustration de la démarche  :

  - **Modèle 1** : 7 variables explicatives (pression moyenne, précipitation, humidité moyenne, nébulosité globale,direction et vitesse de vent à 10m altitude, ensoleillement ) 
    
*code R : choixman_mod1 = regsubsets (pluie.demain ~ Pressure.mean + Precip.tot + Humid.mean + TotalCloud.mean + WindSpeed10m.mean + WindDir10m.mean + Sunshine.tot, int=T,nbest=1,nvmax=7,method="exhaustive",data=d.train)*

```{r choixman1 }
attach(d.train)

# Tests de corrélation 
#cor.test(WindDir10m.mean,WindDir80m.mean)
#cor.test(WindDir10m.mean,WindDir900mb.mean)
#cor.test(WindDir80m.mean,WindDir900mb.mean)

#cor.test(WindSpeed10m.mean,WindSpeed80m.mean)
#cor.test(WindSpeed10m.mean,WindSpeed900mb.mean)
#cor.test(WindSpeed80m.mean,WindSpeed900mb.mean)

# on crée un vecteur de booléens, tirés aléatoirement tel que 80% du jeu de données soit utilisé pour identifier le modèle et 20% pour verifier l'ecart de prédiction
train_80 = sample(c(T, F), nrow(d.train), replace = T, prob = c(.8, .2))

choixman_mod1=regsubsets(pluie.demain~Pressure.mean+Precip.tot+Humid.mean+TotalCloud.mean+WindSpeed10m.mean+WindDir10m.mean+Sunshine.tot,int=T,nbest=1,nvmax=7,method="exhaustive",data=d.train[train_80,])
...
```
  ...
  - **Modèle 2** : 6 variables explicatives : remplacement des variables direction et vitesse de vent à 10m altitude par celles sous 900hPA et suppression de la variable ensoleillement  car les critères CP Mallows et BIC sont non satisfaisants sur ces variables dans le modèle 1
    
    (modèle 2 : pression moyenne, précipitation totale, humidité moyenne nébulosité totale, vitesse du vent à 900hPA)
    

```{r choixman2 }

choixman_mod2=regsubsets(pluie.demain~Pressure.mean+Precip.tot+Humid.mean+TotalCloud.mean+WindSpeed900mb.mean+WindDir900mb.mean,int=T,nbest=1,nvmax=6,method="exhaustive",data=d.train[train_80,])
...
```
  ...
  - **Modèle 3** : 4 variables explicatives
    (presssion moyenne, nébulosité totale, vitesse maximum du vent et direction du vent à 900hPA)
    Les critères du modèle 3 sont satisfaisants pour toutes ses variables.
    
```{r choixman3 }

choixman_mod3=regsubsets(pluie.demain~Pressure.mean+TotalCloud.mean+WindSpeed900mb.max+WindDir900mb.mean,int=T,nbest=1,nvmax=4,method="exhaustive",data=d.train[train_80,])

#plot(choixman_mod3,scale = "adjr2",col="green", main="Modèle 3 - critère R2 ajusté")
...
Dans cette étape, nous allons utiliser les méthodes pas à pas (progressive, descendante et ascendante) pour choisir les variables explicatives du modèle.

Le modèle utilisé sera le modèle logistique : en effet, il permet d'expliquer une variable binaire (présence de pluie ou non demain) en fonction de variables explicatives sélectionnées. En R : *glm ( pluie.demain ~ variables explicatives, family = binomial )*

Commençons par tester le modèle 3 (cf.methode exhaustive)
...
# descendante
# step(glm(pluie.demain~Pressure.mean+TotalCloud.mean+WindSpeed900mb.max+WindDir900mb.mean,data=d.train,family=binomial),data=d[train_80, ],direction="backward")

#progressive
step(glm(pluie.demain~1,data=d.train[train_80,],family=binomial),pluie.demain~Pressure.mean+TotalCloud.mean+WindSpeed900mb.max+WindDir900mb.mean,data=d.train[train_80,],direction="both")

mod3=glm(pluie.demain~Pressure.mean+TotalCloud.mean+WindSpeed900mb.max+WindDir900mb.mean,data=d.train[train_80,],family=binomial)
...
mod0=glm(pluie.demain~1,data=d.train[train_80,],family=binomial)
#aic
aic_mod0=AIC(mod0)
#deviance nulle
...
```
  ...
  - **Modèle 1** : 7 variables explicatives
(pression moyenne, précipitation, humidité moyenne, nébulosité globale,direction et vitesse de vent à 10m altitude, ensoleillement )

```{r choixprog_mod1}
# modèle 1
step(glm(pluie.demain~1,data=d.train[train_80,],family=binomial),pluie.demain~Pressure.mean+Precip.tot+Humid.mean+TotalCloud.mean+WindSpeed10m.mean+WindDir10m.mean+Sunshine.tot,direction="both")

mod1=glm(pluie.demain~Pressure.mean+Precip.tot+Humid.mean+TotalCloud.mean+WindSpeed10m.mean+WindDir10m.mean+Sunshine.tot,data=d.train[train_80,],family=binomial)
...
```
  ...
  - **Modèle 4** : nouveau modèle à 5 variables explicatives . Il combine le modèle 3 en ajoutant humidité moyenne

```{r choixprog_mod4}

# modèle 4 = modèle 3 + Humidité moyenne
step(glm(pluie.demain~1,data=d.train[train_80,],family=binomial),pluie.demain~Pressure.mean+TotalCloud.mean+WindSpeed900mb.max+WindDir900mb.mean+Humid.mean,direction="both")

mod4=glm(pluie.demain~Pressure.mean+TotalCloud.mean+WindSpeed900mb.max+WindDir900mb.mean+Humid.mean,data=d.train[train_80,],family=binomial)
...
```
  ...
  - **Modèle 5** : 4 variables explicatives 
    (modele 3 + température moyenne)

```{r choixprog_mod5}

# modèle 5 = modèle + température moyenne

#progressive
step(glm(pluie.demain~1,data=d.train[train_80,],family=binomial),pluie.demain~Pressure.mean+TotalCloud.mean+WindSpeed900mb.max+WindDir900mb.mean+Temp.mean,data=d.train[train_80,],direction="both")

mod5=glm(pluie.demain~Pressure.mean+TotalCloud.mean+WindSpeed900mb.max+WindDir900mb.mean+Temp.mean,data=d.train[train_80,],family=binomial)
...
```
   ...
   - **Modèle 6 ** = Modèle à 3 variables explicatives . Il reprend le modèle 3 sans la variable vitesse du vent
  
```{r modele6}

step(glm(pluie.demain~1,data=d.train[train_80,],family=binomial),pluie.demain~Pressure.mean+TotalCloud.mean+WindDir900mb.mean,data=d.train[train_80,],direction="both")

mod6=glm(pluie.demain~Pressure.mean+TotalCloud.mean+WindDir900mb.mean,data=d.train[train_80,],family=binomial)

#aic
aic_mod6=AIC(mod6)
#deviance residuelle
deviance_mod6=deviance(mod6)
#degres de liberté deviance résiduelle
dfdeviance_mod6=df.residual(mod6)

```

```{r test}
step(glm(pluie.demain~1,data=d.train[train_80,],family=binomial),pluie.demain~Sortwave.tot, LowCloud.min,WinGustmin, Month, data=d.train[train_80,],direction="both")

mod6=glm(pluie.demain~pluie.demain~Sortwave.tot, LowCloud.min,WinGustmin, Month, data=d.train[train_80,]
        b.mean,data=d.train[train_80,],family=binomial)
...
Quelque soit l'echantillon aléatoire (=80% des individus du fichier d'entrainement), le modèle 3 est meilleur (AIC plus bas que le modele sans variable explicative et que les modèles 1, 4,  5 ou 6)


**Modèle 3 : 4 variables explicatives**

      - **Presssion athmosphérique moyenne** 
      - **Nébulosité totale**
      - **Vitesse maximum du vent à une pression de 900hPA** 
      - **Direction du vent à une pression de 900hPA** 
    
```{r crit}

table_res=data.frame(Modele=c("Modèle nul","Modèle 1","Modèle 3","Modèle 4","Modèle 5","Modèle 6"),AIC=c(aic_mod0,aic_mod1,aic_mod3,aic_mod4,aic_mod5,aic_mod6),Deviance=c(deviance_mod0,deviance_mod1,deviance_mod3,deviance_mod4,deviance_mod5,deviance_mod6),Df=c(dfdeviance_mod0,dfdeviance_mod1,dfdeviance_mod3,dfdeviance_mod4,dfdeviance_mod5,dfdeviance_mod6))
                     
kable(head(table_res), format="markdown", digits=0)

```
    

Les coefficients du modèle 3 obtenu pour la presssion athmosphérique, la nébulosité totale, la direction du vent sont significativement différents de 0 quelque soit le tirage de l'echantillon (p-value < 1%) . La signiciativité du coefficient de la variable vitesse du vent n'est pas toujours vérifiée (pvalue peut être inférieure à 5% ou supérieure à 15% selon l'echantillon)  

```{r affichecoef}

kableExtra::kable(summary(mod3)$coefficients, format="markdown", digits=3) %>%
...
# Modèle 5 
# test par rapport au modèle sans covariable
pchisq0_mod6=pchisq(deviance_mod0 - deviance_mod6, dfdeviance_mod0 - dfdeviance_mod6, lower = F)
pchisq0_mod6
# test par rapport au modèle saturé
pchisqsat_mod6=pchisq(deviance_mod6, dfdeviance_mod6, lower = F)
pchisqsat_mod6
...
anova(mod3, mod6, test = "LRT")


anova(mod4, mod6, test = "LRT")
...
pred1=predict(mod3,d.train[!train_80,],type="response")
mean1=mean(abs(pred1-d.train[!train_80,"pluie.demain"]),na.rm=T)
mean1
#prédiction sur le modèle 6
pred2=predict(mod6,d.train[!train_80,],type="response")
mean2=mean(abs(pred2-d.train[!train_80,"pluie.demain"]),na.rm=T) 
mean2
